resource "azapi_resource" "redis_geo" {
  type                      = "Microsoft.Cache/redisEnterprise@2024-09-01-preview"
  name                      = "redis-geo-${local.base_name}"
...
resource "azapi_resource" "redis_geo_database" {
  type      = "Microsoft.Cache/redisEnterprise/databases@2024-09-01-preview"
  name      = "default"
  parent_id = azapi_resource.redis_geo.id
  body = {
    properties = {
...
resource "azapi_resource" "redis_geo_access_self" {
  type      = "Microsoft.Cache/redisEnterprise/databases/accessPolicyAssignments@2024-09-01-preview"
  name      = "self"
  parent_id = azapi_resource.redis_geo_database.id
  body = {
    properties = {
...
resource "azapi_resource" "redis_geo_access_app" {
  type      = "Microsoft.Cache/redisEnterprise/databases/accessPolicyAssignments@2024-09-01-preview"
  name      = "app"
  parent_id = azapi_resource.redis_geo_database.id
  body = {
